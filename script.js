...
function Rules_CheckIsMovedDiscSmallerThanTargetTower(from_Tower, to_Tower) {
  ...
  if (from_Tower[from_Tower.length - 1] < to_TowerDiscValue) return true;
  else {
...
document.addEventListener("keydown", function (event) {
  ...
  if (event.key == "z") {
    if (fromtower2 != -1) totower2 = TowerNumber.Tower1;
    else {
      fromtower2 = TowerNumber.Tower1;
      fromTowerPreProcess(TowerNumber.Tower1);
    }
  } else if (event.key == "x") {
    if (fromtower2 != -1) totower2 = TowerNumber.Tower2;
    else {
      fromtower2 = TowerNumber.Tower2;
      fromTowerPreProcess(TowerNumber.Tower2);
    }
  } else if (event.key == "c") {
    if (fromtower2 != -1) totower2 = TowerNumber.Tower3;
    else {
  ...
  if (totower != -1) {
    ...
    if (IsTowerEmpty(fromtower)) return;
...
function showSolution() {
  ...
  if (showSolutionSteps < hanoiArray.length) {
    uiUpdate_Feedback("Press same button again");
    moveValue =
      hanoiArray[showSolutionSteps]["from_Tower"] * 10 +
      hanoiArray[showSolutionSteps]["to_Tower"];
    console.log("moveValue=" + moveValue);
    movement(moveValue);
    showSolutionSteps++;
    if (showSolutionSteps === hanoiArray.length) {
      uiUpdate_Feedback("Complete!");
    }
  }
}
